    ...
    <div class="mdf_trajet">
        <div class="subcontainer">
            <div class="lieux">
                ...
                <p><?php
                    $components = explode(",", $existingValues["lieu_depart"]);
                    $first_two_components = trim($components[0]) . ", " . trim($components[2]);
                    echo $first_two_components;
                    ?></p>

            </div>
            <img src="../templates/image/pinnedmap.jpg" alt="fille regardant une carte" id="imgmdf">
            <div class="lieux">
                ...
                <p><?php
                    $components = explode(",", $existingValues["destination"]);
                    $first_two_components = trim($components[0]) . ", " . trim($components[2]);
                    echo $first_two_components;
                    ?></p>
        ...
        <div class="mdf_inputs">
            <input type="time" name="heure" value="<?= $existingValues["heure_dep"] ?>" id="heure" required class="datetimeTRJ"><br>
            ...
            <input type="number" name="places" id="places" value="<?= $existingValues["places_max"] - $existingValues["places_prises"] ?>" min="0" max="5"><br>
            <input type="text" name="prix" id="prix" value="<?= $existingValues["prix"] ?>"><br>

            <input type="hidden" name="modif" id="trajet_id" value="<?= $trajet_id ?>">

            <?php
            form_submit("Soumettre", "Soumettre", FALSE);echo "<br>";
